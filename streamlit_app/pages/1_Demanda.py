    ...
    <div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 1rem;">
        <div>
            ...
            <p style="color: {COLORS['texto_soft']}; margin: 0; font-size: 1.05rem;">
                Estimación de la demanda de pasajeros a 30 días para las 6 rutas
                principales de RutaViva.
            </p>
        </div>
    </div>
    """,
...
with col_ctrl_2:
    modo = st.radio(
        ...
        format_func=lambda m: {
            "evaluacion": "Evaluación: noviembre de 2024 contra datos reales",
            "prediccion": "Predicción: 30 días posteriores al dataset",
        }[m],
        horizontal=False,
    )
...
if modo == "evaluacion" and resultado["valores_reales"] is not None:
    ...
    st.markdown(
        ...
        <div style="background: {COLORS['crema_dark']}; padding: 1rem 1.5rem;
            ...
            <strong>Resumen de la predicción.</strong>
            Para la ruta <em>{ruta}</em>, el modelo estima una demanda media de
            <strong>{media_pred:.0f} pasajeros/día</strong>
            entre el {fecha_inicio} y el {fecha_fin}. Este valor sirve como
            referencia para planear vehículos y personal.
        </div>
        """,
        unsafe_allow_html=True,
    )
...
st.markdown("### 3 · Análisis PCA de los embeddings de ruta")
...
with st.expander("Cómo leer esta visualización", expanded=False):
    st.markdown(
        ...
        **4 dimensiones** que captura sus particularidades temporales.
        ...
        A diferencia de variables definidas a mano, como `base` o
        `peso_fin_de_semana`, estos embeddings se ajustan durante el entrenamiento
        para mejorar la predicción.

        El PCA proyecta esos 6 vectores en 2D. Cuando dos rutas quedan cerca en
        el gráfico, el modelo las está tratando como rutas con un comportamiento
        temporal parecido.
        """
    )
...
st.markdown(
    ...
    <div style="background: {COLORS['crema_dark']}; padding: 1rem 1.5rem;
        ...
        En el reporte, PC1 se interpreta principalmente como la
        intensidad de la <em>estacionalidad semanal</em>, mientras que PC2
        separa las rutas más sensibles a la <em>temporada alta turística</em>
        de las rutas con demanda más regular.
    </div>
    """,
    unsafe_allow_html=True,
)
